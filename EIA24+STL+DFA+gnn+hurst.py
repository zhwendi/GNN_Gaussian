...
class GaussianConv(nn.Module):
    ...
    #
    #
    #     print("Gaussian Kernel:", gauss_kernel)  # 打印高斯核
    #     print("Sum of Gaussian Kernel:", torch.sum(gauss_kernel))  # 打印高斯核的总和
    #
    #
    ...
    #     print("Shape of x_conv:", x_conv.shape)  # 打印x_conv的形状
    ...
    def forward(self, x, alpha=None, beta=None):
        ...
        if beta is None:
            beta = self.beta
        # 使用线性间隔生成卷积核范围（从负到正的范围，更符合高斯分布中心对称特性）
        kernel_range = torch.linspace(-self.node_features // 2, self.node_features // 2, self.node_features,
                                      dtype=torch.float32)

        # 计算高斯核
        gauss_kernel = torch.exp(-((kernel_range - self.alpha) ** 2) / (2 * self.beta ** 2))
        ...
        print("Sum of Gaussian Kernel:", torch.sum(gauss_kernel))  # 打印高斯核的总和
        # 将高斯核调整为适合卷积的格式
        gauss_kernel = gauss_kernel.view(1, 1, -1)  # (1, 1, node_features)

        # 调整输入数据维度以适应卷积操作
        x = x.unsqueeze(1)  # (batch_size, 1, node_features)
        print(x.shape)
        # 进行一维卷积操作，使用合适的填充以保持输出尺寸与输入相同
        x_conv = F.conv1d(x, gauss_kernel, padding=self.node_features // 2)
        print("Shape of x_conv:", x_conv.shape)
        return x_conv.squeeze(1)  # 去掉多余的维度，返回 (batch_size, node_features)
    ...
    #     if beta is None:
    #         beta = self.beta  # 使用可学习的 beta
    #
    #     # 计算高斯核
    #     # 使用标准正态分布公式
    #     range_vec = torch.arange(0, self.node_features, dtype=torch.float32)
    #     gauss_kernel = (1 / (beta * math.sqrt(2 * math.pi))) * torch.exp(-0.5 * ((range_vec - alpha) / beta) ** 2)
    #
    #     # 归一化高斯核
    #     gauss_kernel = gauss_kernel / gauss_kernel.sum()
    #
    ...
    #     # 调整高斯核的形状以适应卷积操作
    #     gauss_kernel = gauss_kernel.view(1, 1, -1)
    #     x = x.unsqueeze(1)  # 调整输入数据的形状
    #     print(x.shape)
    #
    #     kernel_size = gauss_kernel.size(2)
    #     padding = (kernel_size - 1) // 2  # 计算填充
    #     x_conv = F.conv1d(x, gauss_kernel, padding=padding)  # 应用卷积操作
    #     print("Shape of x_conv:", x_conv.shape)  # 打印x_conv的形状
    #
    #     return x_conv.squeeze(1)  # 返回卷积结果
    ...
    # def forward(self, x, alpha=None, beta=None):
    #     if alpha is None:
    #         alpha = self.alpha.item()  # 获取alpha的数值
    #     if beta is None:
    #         beta = self.beta.item()  # 获取beta的数值
    #
    #     # 计算高斯核
    #     size = self.node_features
    #     start = -size // 2
    #     end = size // 2
    #     range_vec = torch.arange(start, end, dtype=torch.float32)
    #     gauss_kernel = torch.exp(-0.5 * ((range_vec - alpha) / beta) ** 2) / (beta * math.sqrt(2 * math.pi))
    #     gauss_kernel = gauss_kernel / gauss_kernel.sum()
    #
    #     print("Gaussian Kernel:", gauss_kernel)  # 打印高斯核
    #     print("Sum of Gaussian Kernel:", torch.sum(gauss_kernel))  # 打印高斯核的总和
...
def train_model(data_path):
    ...
    x = torch.tensor(normalize_data(data_np[0:len(data_np)-72, :]), dtype=torch.float32)

    edge_index = torch.tensor([[0, 0, 1, 1, 1, 2, 2, 2, 2, 4, 4, 4, 5, 5, 5, 6, 7, 9],
                               [7, 10,4,9, 11, 5, 7, 10,12,5,8,11,7,9,11, 8, 10, 11]],
                              dtype=torch.long)

    # edge_index = torch.tensor([[0,7, 0, 1,4,5,1,9,1,2,5,2,2,2,4,4,5,6],
    #                                 [7,10,10,4,5,7,9,11,11,5,9,7,10,12,8,11,11,8]],
    #                           dtype=torch.long)

    edge_attr_pd = pd.read_excel('F:/AAA/GNN_Gaussian/data/EIA24/new/edge_attr_new.xlsx')
    edge_attr_np = np.nan_to_num(edge_attr_pd.to_numpy(dtype=np.float32))
    edge_attr = torch.tensor(normalize_data(edge_attr_np[0:len(data_np)-72, :]), dtype=torch.float32)

    EIA24_data = TensorDataset(x, edge_attr)
    EIA24_loader = DataLoader(EIA24_data, batch_size=168, shuffle=False)
    ...
    num_batches = len(EIA24_loader)

    for batch_idx, (batch_data, batch_edge_attr) in enumerate(EIA24_loader):
    ...
    df_results.to_excel(f'F:/AAA/GNN_Gaussian/data/EIA24/{data_path.split("/")[-1].split(".")[0]}_results_168.xlsx', index=False)

    return avg_final_rmse.tolist(),avg_final_mae.tolist(),avg_final_mape.tolist()

# 训练两个数据集并绘制对比图
data_path1 = 'F:/AAA/GNN_Gaussian/data/EIA24/new/EIA24_new_filled.xlsx'
data_path2 = 'F:/AAA/GNN_Gaussian/data/EIA24/STL/15000/final_data_new_15000.xlsx'
# data_path1 = 'F:/AAA/GNN_Gaussian/data/EIA24/new/EIA24_new_filled.xlsx'
# data_path2 = 'F:/AAA/GNN_Gaussian/data/EIA24/new/EIA24_new_filled.xlsx'
...
rmse_df.to_excel('F:/AAA/GNN_Gaussian/data/EIA24/total_loss_comparison_168.xlsx', index=False)
...
width = 0.35  # 条形图的宽度
plt.bar(x, avg_rmse1, width, label='EIA24', color='#B8D4E9')
# plt.bar([p + width for p in x], total_rmse2, width, label='EIA24_STL', color='#2F7DBB')
plt.bar([p + width + 0.05 for p in x], avg_rmse2, width, label='EIA24_STL', color='#2F7DBB')
plt.xlabel('Column')
...
plt.tight_layout()
plt.savefig('F:/AAA/GNN_Gaussian/data/EIA24/EIA24_test_rmse_comparison_168.png')
plt.show()