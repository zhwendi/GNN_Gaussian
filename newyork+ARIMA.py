...
# 读取数据集并运行模型
df1 = pd.read_excel('data/纽约州各区用电量数据/ARIMA/newyork.xlsx')
results1 = run_model(df1)

print("\n第二个数据集\n")
df2 = pd.read_excel('data/纽约州各区用电量数据/ARIMA/newyork_STL.xlsx')
results2 = run_model(df2)

# 输出结果
print("newyork RMSE：", results1['RMSE'].mean(), "MAE：", results1['MAE'].mean(), "MAPE：", results1['MAPE'].mean())
print("newyork_STL RMSE：", results2['RMSE'].mean(), "MAE：", results2['MAE'].mean(), "MAPE：", results2['MAPE'].mean())

# 保存结果到Excel文件
with pd.ExcelWriter('data/纽约州各区用电量数据/ARIMA/results.xlsx') as writer:
    results1.to_excel(writer, sheet_name='newyork', index=False)
    results2.to_excel(writer, sheet_name='newyork_STL', index=False)
...
plt.bar(index, results1['RMSE'], bar_width, label='newyork', color='#B8D4E9')
plt.bar(index + bar_width, results2['RMSE'], bar_width, label='newyork_STL', color='#2F7DBB')
...
plt.legend()
plt.tight_layout()
plt.savefig('data/纽约州各区用电量数据/ARIMA/newyork_ARIMA_rmse_comparison.png')
plt.show()