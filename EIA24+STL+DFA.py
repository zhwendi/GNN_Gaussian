...
df = pd.read_excel('F:/AAA/GNN_Gaussian/data/EIA24/new/EIA24_new_filled.xlsx')
...
for j in range(df.shape[1]):
    ...
    plt.tight_layout()
    plt.savefig(f'F:/AAA/GNN_Gaussian/data/EIA24/STL/15000/stl_new_15000_decomposition_column_{j}.png')
    plt.close()  # 关闭图形以释放内存
...
final_df.to_excel('F:/AAA/GNN_Gaussian/data/EIA24/STL/15000/final_data_new_15000.xlsx', index=False)
...
for j in range(final_df.shape[1]):
    ...
    plt.suptitle(f'STL Decomposition for Column {j}')
    plt.tight_layout()
    plt.savefig(f'F:/AAA/GNN_Gaussian/data/EIA24/STL/15000/stl_new_15000_column_{j}.png')
    plt.close()  # 关闭图形以释放内存
